...
\begin{figure}[H]
	...
	\caption[Metabolites considered in Bayesian multinomial logistic regression model]{Metabolites with the highest variable importance score given the feature selection method we employed in a previous work \cite{trainor2018}. The Pearson correlation coefficients between metabolite transformed and scaled abundances are shown. \label{fig:include}}
\end{figure}
...
The LOOCV-estimated confusion matrix is shown in Table~\ref{tab:modelRes} for the model without clinical troponin values and Table ~\ref{tab:modelRes2} for the model with clinical troponin values. Group membership was determined by maximum \emph{a posteriori} estimates. Finally, the raw probability estimates for each human subject are shown in Table~\ref{tab:modelRes3}. The LOOCV-estimated sensitivity for detecting and discriminating thrombotic MI was 90.9\% for the model with and without clinical troponin values. Without clinical troponin values, the estimated specificity was 96.3\%, while with clinical troponin values included the estimated specificity improved to 100.0\%. In terms of the detection and discrimination of non-thrombotic MI, the LOOCV-estimated sensitivity was 83.3\% for both models, and the estimated specificity was 88.5\% for both models. Two stable CAD subjects were classified by both models as non-thrombotic MI subjects. Both of these subjects had relatively small estimated probabilities for thrombotic MI. Two non-thrombotic MI subjects were misclassified by both models. The first (subject \#26) was classified as stable CAD by both models. The second subject (\#27) was first classified as thrombotic MI (model without clinical troponin values), and then classified as stable CAD (model with clinical troponin values). This subject appeared to have nearly equal ($\approx 30\%$) probability mass placed over all three study groups. Finally, one thrombotic MI subject was classified as non-thrombotic MI by both models, however the difference between probability mass for thrombotic MI and non-thrombotic MI was slight ($46.3\%$ vs $39.0\%$ in the model without clinical troponin values; $44.8\%$ vs $36.0\%$ for the model with clinical troponin values).
...
\begin{table}[H]
  	\centering
	  	...
	  	\label{tab:modelRes3}
 	\includegraphics[scale=.9]{../Aim3/PatientPred}
\end{table}

\section{Discussion}
The most notable conclusion from the construction of a Bayesian multinomial logistic regression model is that there is evidence that thrombotic MI, non-thrombotic MI, and stable CAD can be discriminated with 
