...
Silva et al. \cite{silva2014} built on the ideas elaborated in \cite{rogers2009}. First, Silva et al. restate the posterior probability of compound label to mass feature assignment in the following more general form (which we have further modified and presented in our notation):
\begin{align}
\begin{split}
p( \mathbfcal{I}|\textbf{X}) &\propto p(\textbf{X}| \mathbfcal{I}) p( \mathbfcal{I}) \\
p(\mathbfcal{I}|\textbf{X}) \propto p(\textbf{X}|\mathbfcal{I}) \cdot p_{rt}(x_m|\mathbfcal{I},\theta_{rt}) p_{iso}(x_m|Z,\theta_{iso})
\end{split}
\end{align}
